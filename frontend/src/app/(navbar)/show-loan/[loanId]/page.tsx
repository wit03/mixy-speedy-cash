...
export default function page({
    ...
    const [state, setState] = useState<{loanPayments: LoanPayment[], principleAmount: number, totalPaid: number}>({
        loanPayments: [],
        principleAmount: 0,
        totalPaid: 0,
    })
    ...
    async function handleGetAllLoanPayment() {
        ...
        }>("http://localhost:3000/loan/list-loan-payments?loanId="+loanId, {
            method: "GET",
        })

        if(!data || data?.loanPayments.length === 0){
            toast.error("You doesn't have any loan payment left")
            router.push("/show=loan")
            return
        }

        if(data?.loanPayments){
            let totalPaid = 0
            data.loanPayments.forEach(item => {
                totalPaid += item.paidAmount
            });

            let principleAmount = 0
            data.loanPayments.forEach(item => {
                principleAmount += item.paymentAmount
            })

            setState(prev => ({...prev, loanPayments: data.loanPayments, totalPaid: totalPaid, principleAmount:principleAmount }))
        }
    ...
    <div className="flex flex-col gap-6">
        ...
        <div className="bg-white w-full p-4 flex flex-col gap-4">
            <div className="flex justify-between">
                <div className="flex flex-col gap-2">
                    <h6 className="text-xl font-medium text-gray-400">Amount being Paid</h6>
                    <h6 className="text-xl font-medium text-purple-500">{state.totalPaid}</h6>
                </div>
                <div className="flex flex-col gap-2">
                <h6 className="text-xl font-medium text-gray-400">Principle</h6>
                    <h6 className="text-xl font-medium text-purple-500">{state.principleAmount}</h6>
                </div>
            </div>
            <div className="w-full bg-gray-300 h-1 relative rounded-xl">
                <div className={`w-[${Math.ceil((state.totalPaid / state.principleAmount) * 100)}%] absolute bg-purple-600 top-0 left-0  z-10 h-1 rounded-xl`}></div>
            </div>
        </div>
            ...
            <div className="flex flex-col  gap-4">
               <div className="flex flex-col gap-4">
                {state.loanPayments.map((item, i) => (
                   (item.paidStatus === "onProcess" || item.paidStatus === "inDept") &&  
                   <div className={`${item.paidStatus === "inDept" && "border border-red-500"} flex gap-3 justify-around p-4 bg-white` } key={i}>
        ...
        <div className="flex flex-col gap-4 p-4">
            <h6 className="text-xl font-normal text-gray-800">Paid Already</h6>
            <div className="flex flex-col  gap-4">
               <div className="flex flex-col gap-4">
                {state.loanPayments.map((item, i) => (
                   item.paidStatus === "paid" && 
                   <div className="flex gap-3 justify-around p-4 bg-white border border-green-500" key={i}>
                   <div className="min-w-[3rem]  min-h-[3rem] relative rounded-full self-center bg-[#E4DFF1]">
                       <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                           <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                               <path d="M12 12C10.35 12 8.9375 11.4125 7.7625 10.2375C6.5875 9.0625 6 7.65 6 6C6 4.35 6.5875 2.9375 7.7625 1.7625C8.9375 0.5875 10.35 0 12 0C13.65 0 15.0625 0.5875 16.2375 1.7625C17.4125 2.9375 18 4.35 18 6C18 7.65 17.4125 9.0625 16.2375 10.2375C15.0625 11.4125 13.65 12 12 12ZM0 24V19.8C0 18.95 0.219 18.169 0.657 17.457C1.095 16.745 1.676 16.201 2.4 15.825C3.95 15.05 5.525 14.469 7.125 14.082C8.725 13.695 10.35 13.501 12 13.5C13.65 13.499 15.275 13.693 16.875 14.082C18.475 14.471 20.05 15.052 21.6 15.825C22.325 16.2 22.9065 16.744 23.3445 17.457C23.7825 18.17 24.001 18.951 24 19.8V24H0Z" fill="#A694CF"/>
                   ...
                   className="cursor-not-allowed p-4 border border-[#A694CF] hover:bg-purple-600 hover:text-white text-gray-800 rounded-md self-center">
                       <h6 className="text-xl font-normal ">Done</h6>
                   </button>
